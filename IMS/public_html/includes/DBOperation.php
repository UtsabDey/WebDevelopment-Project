...
class DBOperation
{
	...
	public function addProduct($cid,$bid,$pro_name,$price,$stock,$date,$total){
		$pre_stmt = $this->con->prepare("INSERT INTO `products`
			(`cid`, `bid`, `product_name`, `product_price`,
			 `product_stock`, `added_date`, `p_status`,`calculate`)
			 VALUES (?,?,?,?,?,?,?,?)");
		$status = 1;
		$pre_stmt->bind_param("iisdisii",$cid,$bid,$pro_name,$price,$stock,$date,$status,$total);
		$result = $pre_stmt->execute() or die($this->con->error);
		if ($result) {
